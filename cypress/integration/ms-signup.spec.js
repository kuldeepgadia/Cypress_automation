...
it('Enter email ID and click Continue button on Signup - STEP_ONE page', () => {
  cy
   ...
   .find('input', { includeShadowDom: true ,timeout: 10000 }).should('be.visible').type('boss87992@healthline.com');
   cy
   ...
   cy.location('href').should('eq', 'https://ms.healthline.com/registration/STEP_TWO');
}); 
...
it('Fill form fields on Signup - STEP_TWO page', () => {
   ...
   .contains('username') .click({force: true});
    ...
    cy
    .get('app-registration.ion-page.hydrated.can-go-back', { includeShadowDom: true })
    .find('input[placeholder="Create a username"]', { includeShadowDom: true ,timeout: 10000 }).should('be.visible').type('boss87992');
